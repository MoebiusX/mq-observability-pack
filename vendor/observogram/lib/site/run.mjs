...
// dashboards, a site.json manifest and the self-checks. Pure: run() writes nothing and returns
// every file as { path, content }; the CLI (tools/gen-site.mjs) reads and writes. Nothing here
// is pack-specific: the pack-specific half arrives as `module` (contract below).
//
...
//   perQmgr(ctx, qm)                   same shape, once per queue manager of the environment
...
//   qmgrs[], hosts[]                    the environment's members (queue managers carry env,
//                                      exporter_host and site)
//   siteOf(qm)                          the site label of a queue manager's hosts
//   burn                                { groups, recording, forecasts, warnings, step } after compileBurnRules
//   manifest                            the site.json object after it is built
//   lib                                 the dashboards library passed to run() (for boards())
// }
//
// Output tree per environment (paths are relative to <out>/<env>/):
//   site.json, packs/<name>.pack.yaml, prometheus/rules/<name>.burn.yml, grafana/dashboards/<file>
//   plus whatever templates/perQmgr return.

import { loadInventories, mergeInventories, validateInventory, resolveEnvironments } from './inventory.mjs';
...
export function selectEnvironments(envs, env) {
  const present = Object.keys(envs).filter(e => envs[e].queue_managers.length || envs[e].hosts.length);
  if (!present.length) return { selected: [], error: 'the inventory has no hosts or queue managers in any environment', usage: false };
  if (env == null || env === '') {
  ...
  if (!present.includes(env)) return { selected: [], error: `--env ${env}: no host or queue manager of the inventory is in that environment (present: ${present.join(', ')})`, usage: true };
  return { selected: [env], error: null, usage: false };
}

/** Normalise a templates() / perQmgr() result to [{ path, content }] rendering functions with ctx. */
...
export function run({ pack, packText, schema = null, inventorySchema = null, inventories = [], env = null, module = null, adapter = null, registry = undefined, repoUrl = null, strict = false, lib = null } = {}) {
  ...
  for (const e of selected) {
    ...
    const ctx = {
      ...
      qmgrs: envModel.queue_managers, hosts: envModel.hosts,
      siteOf: (qm) => envModel.queue_managers.find(x => x.name === (qm?.name ?? qm))?.site ?? null,
    ...
    try {
      ...
      const manifest = {
        ...
        queue_managers: envModel.queue_managers.map(qm => ({ name: qm.name, env: qm.env, shape: qm.shape, hosts: qm.hosts ?? [], address: qm.address ?? null, exporter_host: qm.exporter_host ?? null, site: qm.site ?? null, params: qm.params ?? {} })),
      ...
      // --- templates, per queue manager, dashboards
      for (const f of renderTemplates(module?.templates?.(ctx), ctx, 'templates')) addFile(files, f.path, f.content, perr);
      if (module?.perQmgr) for (const qm of envModel.queue_managers) for (const f of renderTemplates(module.perQmgr(ctx, qm), ctx, `perQmgr(${qm.name})`)) addFile(files, f.path, f.content, perr);
      if (module?.boards) {
