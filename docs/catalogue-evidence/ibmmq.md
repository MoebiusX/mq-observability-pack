...
Exporter job, MQI counts: `ibmmq_queue_mqput_mqput1_count`, `ibmmq_queue_mqget_count`, `ibmmq_qmgr_commit_count` and the other `$SYS` "count" elements are **per-interval deltas** — one value per 10 s publication, not an accumulating total (measured live: a steady 5 msg/s producer shows a flat ~50). mq_prometheus v6 defaults `overrideCType: true`, which stamps them TYPE counter; the collector's `prometheusremotewrite` (`add_metric_suffixes: true`) then appends `_total`, and `rate()`/`increase()` on them is meaningless. This lab sets `overrideCType: false`, keeps IBM's names verbatim as gauges, and derives per-second rates with `sum_over_time(x[2m]) / 120`. `ibmmq_channel_messages`, `ibmmq_channel_bytes_sent` and `ibmmq_channel_bytes_rcvd` (DIS CHSTATUS MSGS / BYTSSENT / BYTSRCVD as mq_prometheus publishes them) are per-interval deltas as well: raw samples for the 5 msg/s orders producer's channel instance read 99, 50, 99, 50, … and never exceed 100, and over a steady hour `sum(rate(...[2m]))` gave 5.06 msg/s against 10.15 for `sum(sum_over_time(...[2m])) / 120` and 10.14 for the apps' own counters (produced + consumed + 2 × canary). An earlier version of this paragraph called the channel counters cumulative and the dashboards used `rate()`; they under-reported by half. (`add_metric_suffixes` stays on: it is what turns the canary's OTLP `mq.canary.attempts` into `mq_canary_attempts_total`.)
...
Known caveat: this lab runs mq_prometheus with `overrideCType: false` (see above), so the `$SYS` count elements are gauges holding per-interval deltas and every rate here is `sum_over_time(x[2m]) / 120`. A deployment that leaves v6's default `overrideCType: true` gets those elements as Prometheus counters with a `_total` suffix; the panels and rules built on `sum_over_time` would then have to be rewritten as `rate()`. The pack's SLIs do not depend on the count elements either way. Conformance C6 lists every `ibmmq_*` name actually present so the report shows the truth for the image/exporter versions under test.
