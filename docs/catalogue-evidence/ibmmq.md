...
Burn threshold = `factor × (1 − objective)`; both windows must exceed it (S12). Lab `for:` is 30 s / 2 m / 5 m by short window (the compiler's production defaults are 2 m / 5 m / 10 m). Slow windows (1 h / 6 h) keep firing for hours after any incident by design; the harness therefore grades symptom alerts (S5) and burn-rate alerts (S6, fast windows FAIL, slow windows WARN) separately. `tools/check-rules.mjs` fails when a policy window or forecast has no matching alert rule and compares the generated per-SLO recording rules symmetrically between pack and stack (name, `slo` label and expression), so a stale copy on either side fails.
