...
## 5. Chaos experiments
Faults are docker-level (`engine: litmus` is nominal — the pack schema has no `docker` engine). Each experiment maps to one SLO and to alert names that exist as Prometheus rules (enforced by `tools/check-rules.mjs`). Expected MTTD values assume 10 s scrape, 10 s exporter poll, 10 s rule evaluation, 10-20 s `for`, ≤ 5 s Alertmanager group wait: worst case ≈ 55 s, target 60 s (120 s for message age, which needs 60 s of age to accrue first). Experiments that also expect `MQCanaryFailing` budget 90 s: the canary's 1 m success ratio needs ~4 failed 10 s probes before it drops below 0.5, plus 20 s `for` (measured live: `IBMMQQueueManagerDown` in 45 s, the canary alert in 105 s with the original 2 m / 30 s settings). `listener-stopped` force-stops both SVRCONN channels as well as the listener: stopping the listener alone leaves established client conversations working and nothing is detected (measured live).
